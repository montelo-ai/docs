...
Montelo will then generate the necessary code to call the LLM.
...
Montelo provides the easiest-to-use logging SDK for LLM DevOps.
...
To get started, simply replace the OpenAI instantiation with Montelo.
...
After this, you should be set! All requests to OpenAI will be logged in Montelo.
...
To nest logs under one trace, use `/` between the names.

```typescript
const trace = montelo.startTrace({ name: "Agent" });
...
await trace.openai.chat.completions.create({
  // this log is now nested under the above Writer log
