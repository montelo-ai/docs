---
title: Introduction
description: Collaborative AI agents for complex task management.
...
<CardGroup cols={2}>
  <Card title="Agent" icon="robot" href="/crews/concepts/agent">
    An entity with a specific role and skills, that can also communicate with
    other agents.
  </Card>
...
Install the `montelo` and `@montelo/crews` packages in your project.

```sh Npm
npm install montelo @montelo/crews
```
...
<AccordionGroup>
  <Accordion title="Using .env file" defaultOpen="true">
    <CodeGroup>
      ...
      import { Agent, Model, Tools } from "@montelo/crews";
...
You can also specify the process in which the tasks are completed, either [sequentially or managed](/).
...
This is the final result of the crew's work. You can also see a detailed log of the agents progress and the output of each task on the [dashboard](https://app.montelo.ai/):

<Tabs>
  <Tab title="Generated Blog Post">
