...
Here is an example using the `GoogleSearchTool` from the `@montelo/crews` package:
...
| `GoogleSearchTool`                         | Provides access to Google Search                   | [read more](/) |
...
```typescript
import { z } from "zod";
import { Tool } from "@montelo/crews";
