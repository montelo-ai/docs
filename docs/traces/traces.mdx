...
```python Python
trace = montelo.trace(name="Example Trace")

trace.log(
    name="Vector DB",
    input=dict(topic=topic)
)
...
span = trace.span(name="Span - 1")
span.log(name="Log - 1.1")
span.log(name="Log - 1.2")

nested_span = span.span(name="Nested Span - 1.3")
nested_span.log(name="Log - 1.3.1")
nested_span.log(name="Log - 1.3.2")

span.log(name="Log - 1.4")

trace.log(name="Log - 2")
```
...
def chat(topic: str):
    ...
    trace.log(name="Vector DB", input=dict(topic=topic))

    completion = trace.openai.chat.completions.create(
        name="Short Joke",
        model="gpt-3.5-turbo",
        messages=[
            {
                "role": "user",
                "content": f"Tell me a short joke about {topic}",
            }
        ]
    )
...
def runner(input):
    joke = chat(input["topic"])
    return dict(joke=joke)

def evaluator():
    return dict(is_funny="Yes")

montelo.experiments.createAndRun(
    name="Example Experiment",
    dataset=dataset.slug,
    runner=runner,
    evaluator=evaluator,
