...
| `model` <span class="red">\*</span> | _String_   | The OpenAI model used by the agent to process and generate text                                                         | N/A       |
...
| `eventCallback`                     | _Function_ | A function that is called when the agent is starts, processes and finishes a task. E.g. you can use this to log outputs | N/A       |
| `maxIterations`                     | _Number_   | The maximum number of iterations the agent will run before stopping (default: 5)                                        | N/A       |
...
Here is an example creating an agent using all the properties:
...
const researcher = new Agent({
  ...
  systemMessage: "Format the findings in an easy-to-understand way for sharing with others.",
  // (Optional) Only hit the model twice per task
  maxIterations: 2,
  // (Optional) Using the callback to log the outputs
  eventCallback: async ({ event, message, task }) => {
    console.log(`Researcher => ${event} - ${task} - ${message});
  },
});
```
